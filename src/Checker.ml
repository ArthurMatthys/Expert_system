...
let rec _check2 (line:(string*int*int) list) (left:bool) (imply:int) (right:bool) : ((int, string) result) = match line with
  | [] -> if left && right && imply <> 0 
    then Result.ok (imply)
    else if imply = 0
      then Result.Error ("No implication symbol found")
  ...
  | ("<",_,_)::("=", _,_)::(">", _,_)::t -> if imply = 0 
    then _check2 t left 4 right
    else Result.Error ("Multiple implications found")
  | ("=", _,_)::(">", _,_)::t -> if imply = 0 
    then _check2 t left 3 right
    else Result.Error ("Multiple implications found")
  | ("=", _,_)::t -> if imply = 0 || left
    then _check2 t true 1 right
    else Result.Error ("Equal found but wrongly placed")
  | ("?", _,_)::t -> if imply = 0 || left
    then _check2 t true 2 right
    else Result.Error ("Question mark found but wrongly placed")
  | (h1,v1,_)::(h2,v2,d)::t -> if (v1 = v2 && h2 <> "!") && imply <> 1 && imply <> 2
    then Result.Error ("Two characters with same type found : \"" ^ h1 ^ "\" and\"" ^ h2 ^ "\"")
    else if v1 < 0
      then Result.Error ("Character not valid : \"" ^ h1 ^ "\"")
      else if imply > 0
        then _check2 ((h2,v2,d)::t) left imply true
        else _check2 ((h2,v2,d)::t) true imply right
  | (h,v,_)::t -> if v < 0
    then Result.Error ("Character not valid : \"" ^ h ^ "\"")
    else if imply > 0
      then _check2 t left imply true
      else _check2 t true imply right

let check_input2 (data:(string*int*int) list list) : ((unit, string) result) =
  let rec check_file (lines:(string*int*int) list list) (facts:bool) (init:bool) (search:bool) : ((unit, string) result) = match lines with
    ...
    | h::t -> if List.length h = 0 
      then check_file t facts init search
        ...
        let (res_check: (int, string) result) = _check2 h false 0 false in
        if Result.is_error res_check 
        then Result.Error ("Error in line \"" ^ (_recreate_line2 h) ^ "\" : " ^ Result.get_error res_check)
...
let check_consistency2 (lst_fact:(string*(bool option)) list) (lst_states:(string*int*int)list): ((unit,string) result) = 
  let unknown_facts = List.filter (fun (fact, case, _) -> if case <> 1 
    then false
    else not @@ List.exists ((=) (fact, None)) lst_fact
    ) lst_states
  in
  if List.length unknown_facts = 0
  then Result.ok ()
  else Result.error (match List.nth unknown_facts 0 with (str,_,_) -> str)
